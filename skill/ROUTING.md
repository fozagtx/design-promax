...
## Step 4 — Quality bar (Vault OTP / design-promax “good”)

Before shipping UI, checklist:

1. **Routed** via this file (surface A–H logged mentally)  
2. **Read 2–4 real source files** (not invented classNames)  
3. **Icons:** `@iconify/react` + `solar:` / `gravity-ui:` only (copy names from sources you opened)  
4. **Tokens:** `bg-content1`, `text-default-500/900`, `border-default-200`, `shadow-small`, `rounded-large/medium`  
5. **Components:** `forwardRef` + `cn` + `displayName` for reusable pieces  
6. **Copy:** human product language; no architecture footnotes  
7. **States:** empty / loading / error / wrong-network (or equivalent) each have a card  
